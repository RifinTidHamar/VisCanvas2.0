...
pair<vector<string>, vector<DNSRule>> DomNominalSet::MTBRGSequential(double precisionThresh, vector<vector<int>>groups, int targetClass)
{
	...
	const double COVERAGETHRESHOLD = 0;//%
	...
	while (true)
	{
		...
		}//End of iterating over list.

		if (ruleSelected != true) // no more rules to select.
		{
			break;
		}
...
vector<double> DomNominalSet::combineNegatedAttributes(DNSRule one, DNSRule two)
{
	...
	{
		one.negatedAttributesIndexes.push_back(one.getAttributesUsed().size() + two.getNegatedAttributesIndexes()[i]);
	}
...
DNSRule DomNominalSet::combineRules(DNSRule rule1, DNSRule rule2, double overlapThreshold, int casesInTarget) 
{
	...
	if (rule1.orIndexes.size() > 0)
	{
		rule1.orIndexes.push_back(rule1.orIndexes.back() + rule2.getCoordinatesUsed().size());
	}
	else
	{
		rule1.orIndexes.push_back(rule1.getCoordinatesUsed().size() - 1);
	}
	combinedRule.orIndexes = rule1.orIndexes;
...
vector<DNSRule> DomNominalSet::combineAndTest(int casesInTargetClass, vector<DNSRule> rules, double overlapThreshold, double minPrecision) 
{
	...
	for (int j = 0; j < rules.size(); j++)
	{
		...
		{
			DNSRule otherRule = rules[i];
			//if (otherRule == curRule) continue;
			combinedRule = combineRules(curRule, otherRule, overlapThreshold, casesInTargetClass);
			if (combinedRule.getTotalCases() > 0)
			{
				rules.erase(rules.begin() + i);
				curRule = combinedRule;
				isCombination = true;
			}
		}
		if (isCombination) rules.push_back(combinedRule);

		else rules.push_back(curRule);
...
vector<DNSRule> DomNominalSet::combineRulesGenerated(vector<DNSRule> allGeneratedRules, int targetClass, int numCasesInTargetClass, double precThresh)
{
	...
	for (int i = 0; i < allGeneratedRules.size(); i++)
	{
		...
		vector<double> curRuleNegatedPositions = curRule.getNegatedAttributesIndexes();
	...
	for(int n = 0; n < complexityPerRuleIndex.size(); n++)
	{
		...
		for (int i = 0; i < complexityPerRuleIndex.size(); i++)
		{
			...
			vector<double> candidateCoordinateNegated = candidateRule.getNegatedAttributesIndexes();
			vector<double> curRuleCoordinateNegated = combinedRule.getNegatedAttributesIndexes();
			...
			if (canBeCombined)
			{
				...
				combinedRule.setNegatedAttributesIndexes(combinedNegatedAttributes);
					...
					for (int y = x; y < currentAttributeCombinantion.size(); y++)
					{
						...
						if (predictedTotal != 0)
						{
							...
							if (precision >= PRECISION_THRESHOLD && coverage >= COVERAGE_THRESHOLD)
							{
								...
								newRule.setNegatedAttributesIndexes(decodedNegated);
...
vector<string> DomNominalSet::tenFoldCrossValidation(int targetClass, vector<vector<int>> groups)
{
	...
	for(int n = 0; n < 1; n++)
	{
		...
		for (int i = 0; i < partitions.size(); i++)
		{
			...
			{
				file->deleteSet(testData.at(j));//Fix sorting first.
			}
			...
			for (int k = 0; k < generatedRules.size(); k++)
			{
				...
				vector<double> negatedIndex = curRule.getNegatedAttributesIndexes();
				...
				for (int j = 0; j < testData.size(); j++)
				{
					...
					for (int valuePos = 0; valuePos < attributesUsed.size(); valuePos++)
					{
						...
						else //Not negated thus must be equal to be classified.
						{
							...
							for (int count = 0; count < curCoordIndices.size(); count++)
							{
								...
								if (attributesUsed.at(curCoordIndices.at(count)) == valuesAtRuleCoordinates.at(curCoordIndices.at(count)))
								{
									isNeverEqual = false;
									break;
								}
					...
					if (isClassifed)
					{
						...
						for (int u = 0; u < casesPredicted.size(); u++)
						{
							if (casesPredicted.at(u) == curCaseID)
							{
						...
						if (curCaseClass == curRule.getRuleClass())
						{
							...
							bool isNotContainted = true;
							for (int u = 0; u < casesCorrectltyPredicted.size(); u++)
							{
								if (casesCorrectltyPredicted.at(u) == curCaseID)
								{
									isNotContainted = false;
									break;
								}
